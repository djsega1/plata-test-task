...
пробелы и приводит к верхнему регистру, затем валидирует — `eur/mxn` и ` EUR/MXN ` не отличаются от
`EUR/MXN` для клиента, и наказывать их как «вне allow-list» (`422`) вместо простой нормализации было
бы неверной классификацией ошибки. Дальше по системе ходит только типизированная пара.

**`POST /quotes/updates`** — тело `{"pair":"EUR/MXN"}`, опционально заголовок `Idempotency-Key`
(до 255 символов — длиннее `400`, без этого предела `quote_updates_idem_idx` рос бы неограниченно
от одного заголовка).
...
`400` — тело не парсится, `pair` пусто, строка не имеет формы `BASE/QUOTE` (`ErrMalformedPair`), или
`Idempotency-Key` длиннее 255 символов. `422` — форма верная, но `base`/`quote` вне allow-list или
`base == quote` (`ErrPairNotAllowed`). `409` — тот же ключ с другой парой; `200` +
...
**Реплей отдаёт исходную заявку в любом статусе, включая `failed`.** Идемпотентность гарантирует один
и тот же результат на один и тот же ключ — не «ещё одну попытку». Клиент, которому нужен настоящий
повторный поход после `failed`, посылает новый ключ (или пустой заголовок); это осознанно, не
недосмотр.
...
{"update_id":"6f1e…","pair":"EUR/MXN","status":"succeeded",
 ...
 "quality":"live","derived":false,"indicative":true,"cached":false}
...
**`cached`** отличает «сходили к провайдеру» от «отдали ещё не протухшую котировку»: ручка называется
«обновить», и без этого поля `succeeded` в ответ на реальный поход и `succeeded` в ответ на
переиспользование 20-часовой давности `daily`-котировки неотличимы для клиента. `true` —
`CompleteSuccessReuse` (переиспользование), `false` — `CompleteSuccess` (свежий поход). Поле есть
...
CREATE TABLE quote_updates (
    ...
    reused          BOOLEAN,                            -- только succeeded; see «cached» в §4
    error_code      TEXT,
    error_message   TEXT,
...
- **Легальность перехода статуса решает домен, а не `WHERE`.** `memory.Repository` перед записью
  валидирует переход через `CurrencyRateUpdateStatus.CanTransitionTo`/`TransitionTo`. В
  `storage/postgres` `UPDATE ... WHERE status = <ожидаемый>` — это сама проверка и сама защита от
  гонки за один круглый поход; `SELECT` (`diagnoseTransitionFailure`) выполняется только при
  `RowsAffected() == 0`, чтобы превратить голое «0 строк» в содержательную ошибку (не найдено /
  неверный переход / конкурентно изменили), не оплачивая его на каждом успешном вызове. `ClaimBatch` —
...
- **`attempts` считает только `pending -> in_progress` клейм, не реклейм реапером.** Он же — бюджет
  `DISPATCH_MAX_ATTEMPTS`, который должен ограничивать повторы против апстрима, а не переживший
...
`DISPATCH_VISIBILITY_TIMEOUT` обязан быть `>= DISPATCH_PASS_TIMEOUT` — `config.Load` проверяет это
при старте и отказывается запускаться иначе. Причина: весь батч клеймится (и получает `locked_at`)
одной операцией, но конкретная строка обрабатывается только когда до неё доходит очередь в пуле — то
есть может пролежать заклейменной почти весь `DISPATCH_PASS_TIMEOUT`, ещё не будучи тронутой. Более
короткий `DISPATCH_VISIBILITY_TIMEOUT` позволил бы реаперу (следующему проходу этой же реплики или
любому проходу другой) реклеймить строку, которая на самом деле ещё в работе — лишний поход к
апстриму и трата бюджета `RateLimiter` на строку, что и так была в процессе.
...
если это печатаемый ASCII не длиннее 100 символов, иначе генерирует `uuid.NewV7()`), эхом отдаёт в
ответе. Без этой проверки клиент без ingress-прокси перед собой мог бы посадить произвольные байты
или раздутое значение в каждую строку лога этого запроса. Это id одного вызова, не `update_id`:
...
`POSTGRES_MAX_CONN_LIFETIME`, `POSTGRES_HEALTH_CHECK_PERIOD` (0 — дефолт `pgxpool`, `max(4, NumCPU)`).
Та же забота о транспорте, что и у `pkg/httpclient`, не то, что должен угадывать `storage/postgres`.
Если `POSTGRES_MAX_CONNS` задан явно и меньше `DISPATCH_POOL_SIZE`, `config.Load` отказывается
стартовать: меньший пул означал бы, что воркеры диспетчера сами исчерпывают соединения, оставляя
HTTP-хендлеры ждать — в проде это выглядело бы как «база тормозит», а на деле было бы опечаткой в
конфиге. `0` (по умолчанию) проверке не подлежит — реальный дефолт `pgxpool` этому коду на этапе
парсинга конфига не известен.
...
Аутентификация и rate limiting на входе, retention журнала (включая `idempotency_key` — ключ не
удаляется и не имеет TTL, `quote_updates_idem_idx` растёт неограниченно; в проде — ретеншен по
`created_at`), трассировка, брокер сообщений, кэш поверх БД, автоматический fallback между
провайдерами (два источника разойдутся в цене, «молча взять другой» отдало бы клиенту цену без
объяснения происхождения), bid/ask и расчётные курсы Banxico/DOF — понадобятся, если по цене начнут
проводить операции, а тогда меняется не адаптер, а модель котировки.

**Миграции выполняются в `main()` при старте** (`postgres.Migrate`, до создания пула), не отдельным
job/initContainer. При rolling deploy это значит, что долгая миграция блокирует старт всех подов и
съедает бюджет `readinessProbe`; сериализация через advisory lock (goose) делает это безопасным
относительно конкурентности, но не быстрым. Осознанный выбор ради простоты (один бинарник, один
`docker compose up` без отдельного job) для сервиса, который в этом задании не разворачивается как
rolling-обновляемый набор подов; для настоящего rolling deploy миграции — отдельный шаг перед
раскаткой новой версии, не часть `main()`.
...
диспетчере (измерено); при паcсе, реально зависшем в работе, `main.go` ждёт `dispatchDone` не дольше
`HTTP_SHUTDOWN_TIMEOUT` (10с по умолчанию) и продолжает останавливаться — брошенные `in_progress`
строки подберёт реапер, доставка at-least-once. Раньше это ожидание было ничем не ограничено (до
`DISPATCH_PASS_TIMEOUT`, 90с по умолчанию) — дольше типичного `terminationGracePeriodSeconds` в
Kubernetes (30с), то есть реального SIGKILL до завершения дренажа; не воспроизведено живым
запуском (процесс простаивал), но следует прямо из кода — исправлено уже без такого сценария.
