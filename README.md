...
в `succeeded` за 10 секунд — то число, ради которого архитектура из `docs/design.md` (очередь в
Postgres, пул воркеров, single-flight по паре) и построена. Измерено `scripts/throughput.sh`:
POST-only нагрузка (`cmd/throughput`, 50 конкурентных воркеров, 20 секунд, `PROVIDER=fake`) против
поднятой связки, затем прямой запрос к `quote_updates`/`quotes` за реальной скоростью дренирования.
Сырые данные по секундам — `docs/perf/`.

| | 15.09, до фикса | 15.09, после фикса | 16.09, сейчас |
|---|---|---|---|
| создано → succeeded | 11928 / 50205 | 49216 / 49216 | **51911 / 51911** |
| зависло в очереди | 38277 | 0 | **0** |
| лучшее окно за 10 секунд | 7845 котировок/10с | 14708 котировок/10с | **28212 котировок/10с** |

15.09: `Dispatcher.Run` заново клеймил батч только по `nudge` или тику, так что бэклог дренировался
строго `DISPATCH_BATCH_SIZE / DISPATCH_TICK_INTERVAL` = 200 котировок/10с независимо от размера
очереди — исправлено (`Run` сам продолжает клеймить, пока батч приходит полным). 16.09: `ClaimBatch`
переписан на два независимых CTE вместо одного `OR`-условия (раньше сортировка всего бэклога перед
`LIMIT` — см. `docs/design.md` §5), и переиспользование котировки (`CompleteSuccessReuse`) перестало
писать в `quotes` на каждый запрос — почти вдвое подняло скорость дренажа. За оба прогона к
провайдеру реально сходили 6 раз (по одному на пару) — single-flight + `stale_after` переиспользуют
одну котировку для всех остальных запросов по паре.
...
от остальных. Внутри одного процесса `RateLimiter.CoolDown` теперь честно останавливает бюджет
целиком, а не только строку, поймавшую `429`, — апстрим сказал «стоп», и это применяется ко всем
парам этого процесса. Между репликами бюджет по-прежнему не общий: реплика B ничего не знает про
`CoolDown` реплики A. Общий лимитер на Redis (или Postgres advisory lock по паре, в рамках уже
имеющейся зависимости) позволил бы репликам координироваться, чтобы N реплик вместе соблюдали один
бюджет и не дублировали поход за одной и той же парой одновременно.
